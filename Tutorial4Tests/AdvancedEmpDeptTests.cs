...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { Emp = e, Dept = d }).SelectMany(ed => grades.Where(s => ed.Emp.Sal >= s.Losal && ed.Emp.Sal <= s.Hisal).Select(s => new { EName = ed.Emp.EName, DName = ed.Dept.DName, Grade = s.Grade })).ToList();
