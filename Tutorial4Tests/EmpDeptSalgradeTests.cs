...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Select(e => new { e.EName, e.Comm }).Where(e => e.Comm != null).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e => grades.Where(s => e.Sal >= s.Losal && e.Sal <= s.Hisal).Select(s => new { e.EName, s.Grade })).ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > emps.Where(x => x.DeptNo == e.DeptNo).Average(x => x.Sal)).Select(e => e.EName).ToList(); 
